  ...
  methods: {
    guardarMatricula() {
      ...
      if (this.accion === "nuevo") {
        ...
        let index = this.matriculas.findIndex(
          (matricula) => matricula.idMatricula == this.matricula.idMatricula
        );
    ...
    listar() {
      this.matriculas = JSON.parse(
        localStorage.getItem("matriculas") || "[]"
      ).filter(
        (matricula) =>
          matricula.alumno.label
            .toLowerCase()
            .indexOf(this.buscar.toLowerCase()) > -1 ||
          matricula.fecha.indexOf(this.buscar) > -1
      );
      this.alumnos = JSON.parse(localStorage.getItem("alumnos") || "[]").map(
        (alumno) => {
                ...
                <div class="card text-center">
                    ...
                    <div class="card-body">
                        <form id="frmMatricula" @reset.prevent="nuevoMatricula" v-on:submit.prevent="guardarMatricula">
                            ...
                            <div class="row p-1">
                            <div class="col-3 col-md-2">
                            <img :src="matricula.comprobante" with="20" height="50">
                            </div>
                            <div class="col-9 col-md-10">
                            <div class="input-group mb-3">
                               <label class="input-group-text" for="inputGroupFile01">Upload</label>
                                 <input accept="image/*" onChage="seleccionarImagen(this)" type="file" class="form-control" id="inputGroupFile01">
                            </div>
                            </div>
                            <div class="row p-1">
